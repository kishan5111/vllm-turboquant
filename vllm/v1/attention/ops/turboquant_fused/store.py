...
Ports the 0xSero/turboquant CompressedKVStore implementation.
"""

import torch
from typing import Optional, NamedTuple

from .quantizer import TurboQuantProd, ProdQuantized
from .kv_cache import quantize_values, ValueQuantized
...
def _flatten_prod_q(pq: ProdQuantized) -> ProdQuantized:
    ...
    return ProdQuantized(
        ...
        residual_norms=pq.residual_norms.reshape(
            -1, pq.residual_norms.shape[-1]
        ).contiguous(),
...
def _flatten_value_q(vq: ValueQuantized) -> ValueQuantized:
    ...
    return ValueQuantized(
        data=vq.data.reshape(
            -1, vq.data.shape[-2], vq.data.shape[-1]
        ).contiguous(),
        scales=vq.scales.reshape(
            -1, vq.scales.shape[-2], vq.scales.shape[-1]
        ).contiguous(),
        zeros=vq.zeros.reshape(
            -1, vq.zeros.shape[-2], vq.zeros.shape[-1]
        ).contiguous(),
        bits=v_bits,
    )
