...
Two-stage quantization for inner products:
...
class TurboQuantProd(torch.nn.Module):
    ...
    def __init__(
        ...
        # Pre-allocate powers buffer for bit-packing (CUDA-graph safe)
        self.register_buffer(
            "_powers",
            torch.tensor([1, 2, 4, 8, 16, 32, 64, 128], device=self.device, dtype=torch.uint8),
        )
    ...
    def _pack_qjl_signs(self, projected: torch.Tensor) -> torch.Tensor:
        ...
        return (signs_reshaped * self._powers).sum(dim=-1, dtype=torch.uint8)

    def _unpack_qjl_signs(self, packed: torch.Tensor) -> torch.Tensor:
        """Unpack sign bits from uint8 to float {-1, +1}."""
        unpacked = ((packed.unsqueeze(-1) & self._powers) > 0).float()
    ...
    def attention_score(
        ...
        scores_qjl = scores_qjl * (
            self.qjl_scale * quantized_key.residual_norms.unsqueeze(-2)
        )
